...
class PowerPointVBAHandler:
    ...
    def _clear_document_properties(self):
        ...
        try:
            # 清除自定义属性
            try:
                props = self.presentation.CustomDocumentProperties
            ...
            try:
                builtin_props = self.presentation.BuiltInDocumentProperties
            ...
            # 解除演示文稿保护
            try:
                self.presentation.Unprotect()
            except:
                pass
            try:
                self.presentation.Unprotect(Password="teacher2024")
            except:
                pass
            try:
                self.presentation.Unprotect(Password="StudentReadOnly2024")
            except:
                pass
            try:
                self.presentation.Unprotect(Password="NoSelect2024")
            except:
                pass
            try:
                self.presentation.Unprotect(Password="TempProtect2024")
            except:
                pass
            
            self.logger.info("演示文稿属性已清除")
    ...
    def remove_all_vba(self) -> bool:
        ...
        try:
            ...
            # 清除 ThisPresentation 的代码
            if "ThisPresentation" in component_names:
                try:
                    ppt_module = self.vba_project.VBComponents("ThisPresentation")
                    code_module = ppt_module.CodeModule
                    if code_module.CountOfLines > 0:
                        code_module.DeleteLines(1, code_module.CountOfLines)
                    self.logger.info("已清除 ThisPresentation 模块代码")
                    component_names.remove("ThisPresentation")
                except Exception as e:
                    self.logger.warning(f"清除 ThisPresentation 代码失败: {e}")
